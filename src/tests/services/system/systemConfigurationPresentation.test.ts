...
describe("systemConfigurationPresentation", () => {
	it("resolve o status completo quando a carteira possui os dois acessos", () => {
		expect(getSystemConfigurationStatusLabel(true, true, true)).toBe("Dono autenticado");
		expect(getSystemConfigurationStatusColor(true, true, true)).toBe("teal");
	});

	it("resolve o status parcial quando a carteira possui apenas um acesso", () => {
		expect(getSystemConfigurationStatusLabel(true, false, false)).toBe("Acesso parcial");
		expect(getSystemConfigurationStatusLabel(false, true, false)).toBe("Acesso parcial");
		expect(getSystemConfigurationStatusColor(true, false, false)).toBe("teal");
	});

	it("resolve o status de leitura quando a carteira nao e dona de nenhum contrato", () => {
		expect(getSystemConfigurationStatusLabel(false, false, false)).toBe("Apenas leitura");
		expect(getSystemConfigurationStatusColor(false, false, false)).toBe("gray");
	});
